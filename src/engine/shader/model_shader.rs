...
use crate::engine::{math::vec3::Vec3, render::scene_renderer::ObjectType, texture::texture::Texture};
...
impl ModelShader {
    pub unsafe fn new(use_tesselation: bool) -> Self {
        ...
        if use_tesselation {
            let tesc_shader = Shader::new("model.tesc", gl::TESS_CONTROL_SHADER);
            let tese_shader = Shader::new("model.tese", gl::TESS_EVALUATION_SHADER);
        ...
        let shader_program = Self {
            ...
            object_type_location: gl::GetUniformLocation(id, std::ffi::CStr::as_ptr(&CString::new("uObjectType").unwrap())),
            //has_texture_location: gl::GetUniformLocation(id, std::ffi::CStr::as_ptr(&CString::new("uHasTexture").unwrap())),
            texture_type_location: gl::GetUniformLocation(id, std::ffi::CStr::as_ptr(&CString::new("uTextureType").unwrap())),
            ...
            terrain_isuphill_location: gl::GetUniformLocation(id, std::ffi::CStr::as_ptr(&CString::new("uTerrainIsUphill").unwrap())),
            terrain_height_location: gl::GetUniformLocation(id, std::ffi::CStr::as_ptr(&CString::new("uTerrainHeight").unwrap())),
            current_location: gl::GetUniformLocation(id, std::ffi::CStr::as_ptr(&CString::new("uCurrent").unwrap())),
        };
