...
use crate::{transformation, model_group};
...
impl PlayerRenderer {
    ...
    pub fn render(&self, scene: &mut Scene, texture_handler: &mut TextureHandler) {
        ...
        unsafe {
            ...
            let model_group = &mut scene.player.model_group;
            let model_group_position = model_group.get_position().clone();
            let model_group_rotation = model_group.get_rotation().clone();
            let model_group_scale = model_group.get_scale().clone();
            for model in model_group.get_models() {
                ...
                //let model = &mut scene.player.model_group.get_model(0);
                let model_matrix = transformation::create_model_matrix(
                    model_group_position.plus(&model.get_position().rotated(-model_group_rotation)),
                    model_group_rotation + model.get_rotation(),
                    model_group_scale * model.get_scale()
                );
