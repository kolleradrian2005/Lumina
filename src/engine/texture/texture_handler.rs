use std::{collections::HashMap, fs::{self, File}, io::Read, path::PathBuf};
...
use super::{font_texture::{FontTexture, OffsetTexture}, texture::{Texture, StaticTexture, AnimatedTexture}};
...
impl TextureHandler {
    ...
    pub fn load_static_texture(&mut self, texture_name: &str) -> Option<Texture> {
        let raw_path: String = (String::new() + references::TEXTURES_PATH + texture_name).to_string();
    ...
    pub fn load_font(&mut self, font_name: &str) -> Option<FontTexture> {
        ...
        if file_opt.is_err() {
            println!("Could not open font file: {}", raw_path);
            return None;
        }
        let mut file = file_opt.unwrap();
        let mut font_data = Vec::new();
        if file.read_to_end(&mut font_data).is_err() {
            println!("Could not read font file: {}", raw_path);
            return None;
        }
        let ft_opt = Font::try_from_bytes(font_data.as_slice());
        if ft_opt.is_none() {
            println!("Could not parse font file: {}", raw_path);
            return None;
        }
        ...
        for c in 1 as char..=255 as char {
            ...
            if bb_opt.is_none() {
                font_texture.set(c, OffsetTexture {
                    texture : None,
                    v_offset: 0.0,
    ...
    pub fn load_animated_texture(&mut self, texture_names: &[&str], animation_time: u128) -> Option<Texture> {
        let mut static_textures: Vec<StaticTexture> = Vec::new();
        for texture_name in texture_names {
            if let Some(Texture::StaticTexture(static_texture)) = self.load_static_texture(texture_name) {
