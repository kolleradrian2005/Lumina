use crate::engine::gui::{gui_element::{CrossAxisAlignment, GuiElement, MainAxisAlignment, UiElement}, ui_model_group::UiModelGroup};
...
impl GuiElement for Column {
    fn collect_models(&self, max_size: (f32, f32)) -> UiModelGroup {
        ...
        let child_model_groups: Vec<_> = self.children
            .iter()
        ...
        fn leftover(child_model_groups: &Vec<UiModelGroup>, max_size: (f32, f32)) -> f32 {
            max_size.1 - child_model_groups
                .iter()
                .map(|model_group| model_group.dimensions.1)
                .fold(0.0, |acc, val| acc + val)
        }

        let space = match self.main_axis_alignment {
            MainAxisAlignment::SpaceBetween => leftover(&child_model_groups, max_size) / (children_count - 1) as f32,
            MainAxisAlignment::SpaceAround => leftover(&child_model_groups, max_size) / children_count as f32,
            MainAxisAlignment::SpaceEvenly => leftover(&child_model_groups, max_size) / (children_count + 1) as f32,
            _ => 0.0
        };
        
        let mut y_offset = match self.main_axis_alignment {
        ...
        for mut child_model_group in child_model_groups {
            child_model_group.add_margin((
                match self.cross_axis_alignment {
                    ...
                    CrossAxisAlignment::End => max_size.0 - child_model_group.dimensions.0,
                    CrossAxisAlignment::Center => (max_size.0 - child_model_group.dimensions.0) / 2.0,
                },
                y_offset
            ));
            ...
            y_offset += child_model_group.dimensions.1 + space;       
        }
